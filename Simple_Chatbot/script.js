...
// API setup
const API_KEY = `AIzaSyBldjKYPE1gmJVBeGPiUV7dCm7eRcdBdSY`;
...
const voiceBtn = document.getElementById('voice-btn');
    ...
    if (SpeechRecognition) {
        ...
        alert('Trình duyệt của bạn không hỗ trợ chức năng ghi âm giọng nói.');
...
function speakText(text) {
        ...
        utterance.lang = 'vi-VN'; // Đổi thành 'en-US' nếu dùng tiếng Anh
    ...
    // Gọi nói khi click nút speaker
    const speakBtn = document.getElementById("speak-btn");
    if (speakBtn) {
        speakBtn.addEventListener("click", () => {
            const lastBotMsg = document.querySelector(".chat-body .bot-message:last-child .message-text");
            if (lastBotMsg) {
                speakText(lastBotMsg.innerText);
            }
        });
    }
    ...
    async function speakText(text) {
        ...
        const voice = loadedVoices.find(v => v.lang === 'vi-VN');
        if (voice) {
